...
public class AutoScaleService extends Thread {
	...
	private PriorityBlockingQueue<ec2check> pq = new PriorityBlockingQueue<>(20,new Comparator<ec2check>() {
		@Override
	...
	public void initializeAndStartInstance(String ami, String instType, Integer minInstances, Integer maxIinstances) {
		try {
			...
			for(int i=minInstances;i<=maxIinstances;i++) {
				RunInstancesRequest instancesRequest = getAwsAllTagsAndRunInstances(ami, instType, 1,1);
	...
	public Integer noOfec2RunningNow() {
		...
		System.out.println("No.Of Running + pending initially : "+num);
